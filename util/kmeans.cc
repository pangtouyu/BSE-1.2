...
namespace Util
{
  ...
  {
      Util::Message::startBlock("kmeans",4);
      ...
      else
      {
          ...
          Util::Message::debug("assign membership from recursive call",4);
          for (int i = 0; i < coarseNumPoints; i++)
          {
            const int j = pick(i);
          ...
          Util::Message::debug("assign remaining points to nearest centroid",4);
          for (int i = 0; i < numPoints; i++)
          {
            if (membership(i) != -1)
            {
      ...
      Util::Message::debug(String("kmeans: n=%d d=%d k=%d it=%d",
                                  numPoints,numDim,numClusters,numIterations),4);
      ...
      Util::Message::endBlock(4);
      Util::Message::debug(String("RMS error = %f",err),4);
      return err;
  }
